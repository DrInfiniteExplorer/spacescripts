    ...
    static private FrameText frameText = new FrameText();
    ...
    private class WaypointFollower {
        ...
        public void Go(Vector3D currentWorldPos, MatrixD worldMatrix, Action<string, Waypoint, List<OnArriveAction>> f) {
            var distanceToNextWaypoint = (Current.WorldPos - currentWorldPos).Length();
            frameText.Writeln($"Dist: {distanceToNextWaypoint}");
            frameText.Writeln($"Radi: {Current.Radius} Speed: {Current.ApproachSpeed}");
            if (distanceToNextWaypoint < Current.Radius) {
            ...
            Vector3D delta = Vector3D.TransformNormal(Current.WorldPos - currentWorldPos, worldToShipCoords);
            var currentVelocity = Vector3D.TransformNormal(controller.GetShipVelocities().LinearVelocity, worldToShipCoords);
            string yolo = str((Vector3D)delta);
            frameText.Writeln($"Delta: {yolo}");
    ...
    private class ThrustController {
        ...
        public void ApproachVelocity(Vector3D currentVelocity, Vector3D desiredVelocity) {
            ...
            frameText.Writeln($"VDelta: {str(velocityDelta)}");
        ...
        private void ControlThrusters(double velocityDeltaComponent, List<IMyThrust> positiveThrusters, List<IMyThrust> negativeThrusters, double approachSpeed) {
            ...
            foreach (var t in positiveThrusters) {
                ...
                frameText.Writeln($"Pos: {t.ThrustOverridePercentage:F2}");
            }
    ...
    private class FrameText {
        ...
        public void PreludeClear() {
            preludeStringBuilder.Clear();
        }
